...
const KIND_ACCENTS: Record<MonumentKind, readonly MatId[]> = {
  officeTotem: ['shelfMetal', 'upholstery', 'lockerGreen', 'signPlate', 'neonRed'],
  steel: ['redShutter', 'metalDark', 'neonRed', 'stainless'],
  stoneFrame: ['marbleWhite', 'stainless', 'columnConcrete', 'neonBlue', 'goldTrim'],
  cubeCluster: ['columnConcrete', 'stainless', 'wallWhite', 'glass', 'neonBlue'],
  ribbon: ['marbleWhite', 'stainless', 'glass', 'neonBlue'],
  colorStack: ['plasticRed', 'plasticYellow', 'plasticBlue', 'lockerGreen', 'neonRed', 'glass'],
};
...
function amplify(b: MonumentBuild, rng: Rng, o: MonumentOptions, kind: MonumentKind): void {
  ...
  const k = 0.6 + o.distort * 0.8; // 増幅の度合い
  ...
  for (let i = 0; i < nEcho; i++) {
    ...
    const scale = rng.float(0.5, 1.3);
    const size: Vec3 = [src.size[0] * scale, src.size[1] * scale, (src.size[2] ?? 0) * scale];
  ...
  // 3. 軌道の円環 + 球
  const nRing = rng.chance(0.75) ? (rng.chance(0.4) ? 2 : 1) : 0;
  for (let i = 0; i < nRing; i++) {
    ...
    for (let s = 0; s < nSph; s++) {
      ...
      push({ prim: 'sphere', mat: rng.chance(0.6) ? 'stainless' : pickOf(rng, accents), pos: [cx + Math.cos(t) * rr, y + rng.float(-0.2, 0.2) * rr, cz + Math.sin(t) * rr], size: [rr * rng.float(0.1, 0.22), 0, 0] });
    }
  }
  // 4. 持ち出しの梁 + 先端の重い立方体
  const nCant = rng.chance(0.8) ? rng.int(1, 2) : 0;
