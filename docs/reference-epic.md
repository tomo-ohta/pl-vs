...
| E08 | 内部 +41 / +49（殻 0） | +664 / +844 | 0 | 0 | 0 | 内部 56 → 71 ms（遠景の灯 12 のとき +27%）→ 灯を 9 に減らして再計測 43 ms（焼き込みの揺れの範囲） |
...
### E08 内部拡張会議室（SmallRoom + NonEuclideanVolume）— 2026-09-21 A1 で実装
- 契約: NonEuclideanVolume は殻・内部の両方で `L.boxes` を組み直すが、`kind` が `'dress:'` で始まる箱は残す（内部では中心 × s = sqrt(interiorScale) に写し、寸法はそのまま。天井から 0.35 m 以内のものは天井の高さの差だけ上げる。殻の内側に clip）。ドレッシングは内部ノード（`p.node.role === 'interior'`）に限り、NEV の家具を同じ式で先読みして「拡大後に置きたい位置 ÷ s」に置く（`dressE08` の `pre()`）。殻ノードには何も置かない。
- 実装: 会議机（NEV の長机、内法 1.6 m の中央）の上にノート PC 8 台（stainless の台 0.32 × 0.22 + 液晶 0.30 × 0.20。1 台おきに点灯 screenLcd / 消灯 screenDark、点灯 ≤ 4）。NEV のホワイトボード（入口の対面、キャビネット壁なら 0.45 手前 = 同じ fork `mod:NonEuclideanVolume` → `cabinets` を先読み）に stainless の枠とマーカートレイ、青・赤のマーカー。側壁（入口・ホワイトボードの壁を除き、キャビネットの無い壁を優先）にガラス窓: void の奥板 + 遠くの点灯（windowLit 0.16 × 0.10、≤ 9）+ glass + stainless の桟（1.5 m）・枠・窓台。開口 ± 1.2 m を避けた最長区間、長さ ≤ 9 m、キャビネット壁では窓台 1.25 m。
- 見えたもの（`E08-s7-after-table.jpg` / `E08-s7-after-window.jpg`）: 長机の両側に銀のノート PC（青白い画面と暗い画面が交互）、奥のホワイトボードに枠、右壁のキャビネットの上に暗いガラス窓と点々の灯。
- 参考との差: 「ノート PC の台は furnitureLight」という当初案は天板（furnitureLight）と同色で見えないので stainless にした。窓の奥行きは数 cm の見せかけ（glass の反射で暗い空間に読める）。
...
- E08: 解消（`kind: 'dress:*'` の箱を NEV が残す契約。上記）。殻ノードには何も置かない方針のまま。
