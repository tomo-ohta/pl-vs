...
## 追記（2026-09-22 統合）: VHS の強化と既定の変更

- 既定を `tape`、REC 表示を on に変更（`Settings` のキーを v2 に上げ、v1 から音量・感度・Tier だけ移行）。
- VHS の質感を出す要素を再検討し、次を追加した（`VideoParams` / `VideoShader`）: **輝度の横ぼかし**（`lumaBlur`。VHS の輝度帯域 ≈ 240 本の甘さ）、**色差の遅れ**（`chromaShift`。色が輪郭より右に出る）、**明部の右への滲み**（`smear`。テープの smear、暖色寄り）、**縁のリンギング**（`ringing`。強調回路の出過ぎ）、**行ごとの横揺れ**（`lineJitter`。トラッキングの甘さ、下端ほど強い）、**ドロップアウト**（`dropouts`。白い短い筋）、**スノー**（`snow`。暗部の白い点）、**常時のトラッキング帯**（`tracking`。下端 0〜3%）、**行ごとの色相ずれ**（`chromaNoise`）。
- tape プリセット: 色差ぼかし 7 px・遅れ 3 px、輝度ぼかし 2.2 px、滲み 0.7、リンギング 0.55、行揺れ 1.2 px、ドロップアウト 0.5、スノー 0.3、トラッキング帯 1.8%、彩度 −18%、黒浮き 0.03、ニー 0.7、走査線 0.55、コーミング 0.5、間引き 30 fps。残像（frameBlend）は 0.3 → 0.15（過去フレームの残りを弱く）。homeVideo にも弱い値を入れた。
- 静止 3 s 超はドロップアウトを止め、行揺れを 0.3 倍にする（粒子とスノーは残る）。


## 追記（2026-09-23）: 白い横線の削除・ビネット・強化
- 画面に白い横線が出る処理（ドロップアウトの筋、トラッキング帯とヘッド切替の帯の上端の 1 px の線）を削除。`dropouts` パラメータは廃止。
- 四隅の減光 `vignette` を VideoPass（表示域）に追加: 楕円 smoothstep(0.32, 1.0) で四隅だけ暗くする。clean 0.12 / homeVideo 0.2 / tape 0.28。
- tape をさらに強く: 色差ぼかし 8 px・遅れ 3.5 px、輝度ぼかし 2.6 px、滲み 0.85、リンギング 0.65、行揺れ 1.5 px、粒子 0.036、彩度 −20%、黒浮き 0.035、ニー 0.75、走査線 0.6、コーミング 0.55、揺れ 0.35、ヘッド切替 0.4、トラッキング帯 2.1%、色相ずれ 0.6。

## 追記（2026-09-23 その 2）
tape をさらに強く: 色差ぼかし 10 px・遅れ 4.5 px、輝度ぼかし 3.2 px、滲み 1.0、リンギング 0.8、行揺れ 2.0 px、粒子 0.046、彩度 −24%、黒浮き 0.045、ニー 0.85、走査線 0.7（深さ 0.15）、コーミング 0.65、揺れ 0.45、ヘッド切替 0.5、トラッキング帯 2.6%、色相ずれ 0.75、スノー 0.45、四隅の減光 0.32。

## 追記（2026-09-23 その 3）: VHS 効果のスライダー
設定「VHS 効果」（0〜200%、既定 100% = プリセットの値そのまま）。`VideoPass.strength` として frameHold 以外の全効果（彩度・にじみ・粒子・黒浮き・ニー・走査線・コーミング・揺れ・ヘッド切替・DCT・残像・輝度ぼかし・色の遅れ・滲み・リンギング・減光・行揺れ・スノー・トラッキング帯・色相ずれ・白の偏り）に掛け、効果ごとの上限でクランプする。0 で全部オフ（間引きは残る）。

## 追記（2026-09-23 その 4）
- 既定の VHS 効果を 200% に（設定キー v3。v2 からは VHS 効果以外を移行）。
- 焼き付きの修正: 強さ 200% でコーミング（奇数行に前フレームを混ぜる割合）が 1.0 に達し、奇数行が最初のフレームのまま更新されなくなっていた。上限を 0.8 に、残像の上限を 0.4 にした。

## 追記（2026-09-23 その 5）
残像を弱く: 間引きの残像 frameBlend 0.15 → 0.06（上限 0.2）、コーミング 0.65 → 0.4（上限 0.6）。白点のスノー 0.45 → 0.15（上限 0.5）、homeVideo 0.1 → 0.05。
