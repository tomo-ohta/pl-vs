...
| U11 手荷物受取所 | 汎用家具を外し（PropRepetition も捨てる）、床をタイルに、柱の間に収まる段付きステンレスのターンテーブル（台座 0.72 + **黒ゴム（rubber）のベルト面** + 段状の島 + 投入口。5.4 × 3.6 → 4.2 × 3.0 → 3.6 × 2.8 の順に置き場を探す）と**スーツケース 2〜3 個（plasticRed / furnitureDark / seatRed）**、上に「3」の黄色い吊り看板、北壁に「3 手荷物受取所 / Baggage Claim」。ターンテーブルの箱は全て `kind: 'dress:carousel'`（PropRepetition の removeInterior が捨てない印。A1 で契約化）。周囲のリングコンベア + 多数のスーツケースは PropRepetition | 箔 +34〜40（家具 −13 込み） |
...
| U15 単一商品スーパー | 消灯パネルを点け高照度に、全通路に同じ「5 日用品 / Daily Goods」の吊り看板（両面）。**棚の縁の値札レール（非ソリッドの白い薄箔 signPlate、`kind: 'dress:rail'`。棚板 level = 0.25 + k·max(0.9, h/3) の前縁に高 0.05・出 12 mm。長い面 2 面 × 段、上限 260）**。商品（白いボトル = PropRepetition の params `mat: 'signPlate'`, `size: [0.08, 0.24, 0.08]`。細い商品は箔 2 枚を 45° ずらした 8 角柱近似）は Modifier | 箔 +86〜189、サイン +12 |
...
| U11 | 中央に段付きステンレスのターンテーブル（黒ゴムのベルト・島・投入口・スーツケース 2〜3）と黄色い「3」の吊り看板、周囲に PropRepetition のリングコンベア + 多数のスーツケース（`U11-s7-carousel.jpg` → A1 後 `U11-s7-after.jpg`） | 2 種のコンベアが同居する。床は白系タイル（参考は暗い） |
...
| U15 | 同じ白いボトル（signPlate 0.08 × 0.24、8 角柱近似）が全棚の 2 段に並ぶ、棚の前縁の白い値札レール、各通路に「5 日用品」の青い吊り看板、白い高照度（`U15-s7-aisle.jpg` → A1 後 `U15-s7-after.jpg`） | ボトルは 2 枚箔なので 1 InstanceSpec あたり 6,000（合計 12,000）で間隔が広がる部屋がある。レールは棚板（shelfMetal）とのコントラストが低い |
...
- U15（解消・2026-09-21 A1）: PropRepetition sameProduct が params `mat` / `size` を受け付け（rooms.json は signPlate 0.08 × 0.24 × 0.08 = 白いボトル）、`kind: 'dress:*'` の箔は removeFills が捨てなくなったので、値札レールを非ソリッドで足した。構築時間 seed 7: 209 → 237 ms（+13%。ボトル 5,336 → 12,000 個、箔 190 → 450）。
- U11（解消・2026-09-21 A1）: ターンテーブルの箱に `kind: 'dress:carousel'` を付け、ベルトを rubber、スーツケースを plasticRed / furnitureDark / seatRed で 2〜3 個に。構築時間 seed 7: 103 → 89〜132 ms（焼き込みの揺れの範囲）。PropRepetition luggage の density は rooms.json のまま（参考の「スーツケース 1 つ」に寄せるなら density を下げる = データ側）。
