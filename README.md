...
- **設定**（`src/core/Settings.ts` / `src/ui/SettingsPanel.ts`）: 音量 3 本・視点感度・品質 Tier・描画効果（撮像プリセット。既定 homeVideo）・手持ち感（0〜1。既定 0.6、0 で無効）・視線の遅れ（既定 on）・表示 fps（プリセットに従う / 30 / 24 に間引く。tape 以外でも指定可）・REC 表示（既定 off）・トーンマップ（開発用）を `localStorage 'liminal.settings.v1'` に保存し起動時に復元。メニューの「設定」パネル（`#settings-slot`）。カメラ挙動の数値は `docs/film-camera.md`。
