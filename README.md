...
| Space ジャンプ / Shift ダッシュ | 右下 ダッシュ（走る人）・ジャンプ（腕を上げて跳ぶ人と足元の影）ボタン |
| 左 Ctrl または C しゃがみ（押している間） | 右下 しゃがむ（低い天井の下で、前足の膝を立て後ろ足をつま先立ちにして深くしゃがむ人。ジャンプの上。タップで切替、切替中は黄色に点灯） |
| E で扉・エレベーターを操作、乗車中は E で到着を早める（5 秒後） | 扉やボタンを直接タップ |
| Esc メニュー（タブ 1 / 2 / 3・Q / E、マップは ↑↓ で階・M で平面 / 3D） | 右上 メニュー（三本線。●REC の下） |

スマホのボタン（`index.html` #touch-ui / `src/style.css` .tbtn）は塗りの円を使わず、ビデオカメラの画面の四隅の枠 + 動作の人型ピクトグラム（非常口の案内表示と同じ系統の SVG）+ 下に小さな文字（REC・タイムコードと同じ等幅）。明るい場面でも読めるよう影を付ける。
...
- **見えない壁・進めない部屋の修正（第19回。L12 設備大聖堂 seed 2559531081 の報告）**:
  - 原因 1（`src/modifiers/mods/ScaleAnomaly.ts` perProp）: 内装を 3〜8 倍にするとき、見た目を持たない当たり判定（kind 'colliderOnly'。モニュメント・乱れの物）も拡大していた。見た目の MonumentSpec は拡大されないので、巨大モニュメントの判定が 33 m 四方に膨らみ部屋の中央に見えない壁ができていた → colliderOnly は拡大しない（倍率の乱数は消費して他の家具の倍率を変えない）。
  - 原因 2（同）: 倍率を「床の範囲が同じ箱」ごとに決めていたので、1 脚の椅子の座面・脚・背もたれが別々の倍率になり、描画側（部品の組から描き直す）と当たり判定がずれて背もたれの判定が 1.8 m の見えない板になっていた → 家具の部品の組（propGroup）は組ごとに同じ倍率。
  - 原因 3（同）: 拡大した設備どうしがすき間を埋めて 40 m 級の帯になり、壁際の出口へ行けなかった → 拡大前の配置で入口から各出口まで歩ける最短の道（0.25 m 格子、半径 0.36 m、`walkPaths`）を求め、拡大した物がその道に掛かるなら倍率を下げる（元の大きさなら必ず空く）。
  - 原因 4（`src/generators/monument/index.ts` trimColliders）: 種類ごとの当たり判定が組み立て直後の部品に合わせたままで、増幅・fitEnvelope で部品を動かす / 捨てると元の場所に見えない判定が残っていた（steel で最大 29 m²）→ 最後に「当たり判定 × 部品の外接箱」の積へ切り詰める（覆われていれば 1 つにまとめる。22 cm 角未満の破片は作らない）。
  - 部屋ごと拡大（mode 'room': R13 / L17 / M12）ではモニュメントも ×s（当たり判定だけ拡大されていた）。
  - E08 内部拡張会議室（`src/modifiers/mods/NonEuclideanVolume.ts` dropOddObjects）: 殻・内部とも家具を作り直すとき、当たり判定（colliderOnly）は落ちるのにモニュメント・乱れの物（L.monuments）は残り、殻では部屋の外に、内部では当たり判定の無いまま描かれていた → instances と同じく作り直しで捨てる（刻印の看板も。モニュメントの刻印は SignSpec.id が `monument:` で始まる）。
  - 計測（`tools/seam-stats.mjs`）: 「見た目から外れた見えない当たり判定」（colliderOnly がモニュメントの部品・並べて描く物の外形に入っていない数。1 m² 以上を stderr に `[collider?]`）を常に出す。`--reach` で「入口から床の高さの各扉まで歩けない部屋」（`src/generators/reach.ts` の近似。stderr に `[reach?]`）と「扉への道を塞いで取り消した奇妙さ」（stderr に `[undone]`）。修正前 → 後: 見えない判定 302 → 156 個（最大 40 m² → 2.6 m²。残りは乱れで少し回して倒した物の外接箱の角）、L12 の歩けない部屋 3 → 0。`REACH_DUMP=<roomId,...>` で該当部屋、`REACH_DUMP=bad` で歩けない扉のある部屋の layout を scratch/ に書き出す。
- **歩いて行けない扉（第19回の続き。`--reach` の 10 seed × 100 部屋で 31 件 → 0 件）**:
  - 判定（`src/generators/reach.ts` reachDoors）: 0.1 m 格子（大部屋は粗く）・半幅 0.35 m の四角・セルごとの立つ高さ（接地した箱の上面。隣へは 0.35 m まで登れ、降りるのは自由。浮いた箱は踏み台・台の続き・くぐれない（0.85 m 未満）・頭上に分ける）。シェル側の箱（Mega の間仕切り・階段）も見る。instances の当たり判定は pos.y が底。跳び乗り・乗り物・可動壁は見ない。旧判定（0.25 m 格子・高さ無し）の 31 件のうち 8 件は格子の粗さ（0.74〜0.79 m の隙間。R17 corridorTaper・C15・U09・U13 など）で歩ける。
  - 間仕切りの開口（`common.ts` patternPartitions。U14 / E09 / R16 の OfficeGrid、U11 / U17 の LargeRoom、R12 / E14 の Gallery）: 開口の位置を乱数のまま決めていて、先に置いた机の列・柱が開口の真裏に来る（机と間仕切りの間 0.49 m）と通れなかった → 乱数の位置から 0.25 m 刻みで左右に探し、開口を通る帯（±0.45 m × 前後 1.6 m）に家具の無い位置に開ける。開口とその前後は `RoomLayout.passages` に予約し、後段（free() の島・柱・壁際家具、ドレッシング・奇妙さの zones = `keepOutZones`、PropRepetition の通路判定、FakeSignage のブース）が扉前と同じく避ける。壁片は kind 'partition'。
  - 奇妙さ（`oddity/index.ts`）: 1 つ置くたびに、置く前に届いた扉へまだ届くかを reachDoors で見て、届かなくなったらその奇妙さを取り消して次の候補を試す（巨大モニュメント・空き床のモニュメントも同じ）。canPlace の動線帯は入口→出口の直線しか守らないので、段ボールの格子（space.regularArray。C11 のロッカーとベンチの間の通路）・乱れの散乱（R03・C15。部屋では動線帯を見ない）・廊下の置きっぱなしのカート（R17。反対の壁際のベッドは instances なので canPlace に映らない）・劇場の中央通路の柱（R09 layout.pillars）が通路を閉じていた。取り消しは 10 seed で 9 部屋（regularArray 6 / 乱れ 2 / カート 1 / 柱 1）。控えは浅い複製（乱れの propGroup 付け替えと transforms の splice だけ別に控える）。
  - U11 手荷物受取所（`dressing/uncommon.ts` u11）: 汎用の間仕切りの間の帯をターンテーブルとコンベアの環（PropRepetition luggage）が横切って塞いでいた → 受取所は 1 つの広間として間仕切りを外す（開口の予約も消す）。
  - L02 室内海洋の桟橋（`ShallowWater.ts` buildPier）: 脚の矩形を端で半幅延ばしていたので、幅 1 m の入口扉の前庭（奥行 1.3 m）をデッキが覆い、扉の真ん前が 0.6 m の段になって跳ばないと乗れなかった（縁 0.3 m はデッキの下）→ デッキは前庭（と中間段）の奥から始める。扉 → 縁 0.3 → デッキ 0.6 と登れる（ブラウザで歩いて確認）。
  - L12 設備大聖堂（ScaleAnomaly walkPaths）: 拡大前の道がシェル側の側廊の制御室の壁を突き抜けて引かれ、実際に歩く棚の間の隙間を守れなかった → シェル側の箱も見る。
  - 生成の負荷: 25 部屋の計測で 15 s → 17 s（奇妙さ 1 つにつき判定 1 回。外周の格子は足跡ごとに覚える）。
- **草木は通り抜けられる（第18回。`src/render/RoomBuilder.ts` PASSABLE_VEGETATION）**: 材質 plant / plantLeaf / grass / wheat の箱と並べて描く物（葉の塊・生垣・プランターの球・樹冠・下草・麦）は当たり判定を作らない（R07 地下温室・L14 温室都市などで草木に阻まれず歩ける）。配置の判定では solid のまま（他の物は草木に重ならない）。鉢・植え床（plantSoil）・幹（木材）・棚は当たる。
...
- **AudioEngine**: WebAudio 手続き合成の単一入口。Game が 1 つ所有（`game.audio`）。開始クリックで `unlock()`、入室で `setRoom(def, layout, tier, { roomId, hints, wet })`（audioPreset ラベル → 合成レイヤー、Sabine 近似の残響 5 段階、床材で足音）、毎フレーム `setListener / update`。ボイス予算 low 8 / mid 12 / high 20。`public/audio/manifest.json` にファイルがあれば合成レイヤーを差し替える（無ければ合成フォールバック）。
  - **足音・動作音（第20回。CC0 の録音素材）**: `node tools/build-footsteps.mjs` が `assets/cc0/audio/src` の CC0 素材（kenney.nl Impact Sounds / RPG Audio、OpenGameArt「100 CC0 SFX」「100 CC0 SFX #2」「Fantozzi's Footsteps」。いずれも git 管理外）から床種ごとの一歩の音を作り、`public/audio/steps/`（mp3 mono 約 110 本・約 700 KB。git 管理外、デプロイ前に生成）と `index.json` に書く。AssetManifest が `steps/index.json` も読み、`Sfx.playFootstep / playLand` は素材があれば録音（直前と別の変種・音程 ±6%・音量 ±1.5 dB、走り = 高く大きく、しゃがみ = 丸めて小さく）、無ければ合成。床種は 13（carpet / lino / tile / concrete / wood / metal / grass / snow / gravel / asphalt / ice / water / wet）。素材の無い床は加工で作る（tile = 石畳・コンクリートの高域 +6 dB、lino = 柔らかく、metal = コンクリート + 金属の打音、asphalt = コンクリート + 砂、water = 濡れた一歩 + 水しぶき、ice = タイルを少し高く）。
    - 足元の面（`Game.surfaceUnderPlayer`）: 部屋の床ではなく、足の真下のソリッドな箱のうち上面が足の高さに最も近いものの材質で鳴らす（金属の足場・木の台・棚板など）。床に貼った薄い板（氷・雪・敷物・水たまり）は優先、見えない当たり判定（void）は無視、動く床の上はゴム、水ゾーンの中は水。
    - 新しい音: ジャンプの踏み切り（衣擦れ + 蹴る一歩。`AudioEngine.jump`）、通り抜けられる草木の中の一歩ごとの擦れ（`AudioEngine.rustle`。膝より低い下草は弱く）。
    - Sonniss の GDC Game Audio Bundle は配布サーバーが Cloudflare のボット確認で自動取得できないため未使用（手動で取得して差し替える余地）。
    - 2 回目の調整（ユーザー指摘: 聞こえない場所がある / 鳴ったり鳴らなかったり / 「コツコツ」と「パキパキ」が二重）。測定で原因を特定: ① 同じ床に Kenney の鈍く短い打音（20 ms・500 Hz）と Fantozzi の石畳（200 ms・3 kHz）を混ぜ、一歩ごとに入れ替わっていた → 床ごとに素材を 1 系統（硬い床は Fantozzi の石畳から加工、木は OpenGameArt の木だけ）。② ピーク揃えで短い音が小さく、同じ床で最大 9.8 dB ばらついた → 体感の音量（最大 50 ms の RMS）で -13 dBFS に揃え、短い音は +3 dB（多くの床で幅 1 dB 以内）。一歩ごとの揺れは ±0.6 dB。③ 部屋ごとに環境音が 20 dB 以上違い、空調の部屋では足音が環境音 +1.5 dB しかなかった → AudioEngine が ambientBus を実測（0.2 s ごと、τ 1.5 s）し、足音・着地・ジャンプを最大 +14 dB 自動で持ち上げる（`stepBoost`。静かな部屋は 0 dB）。環境音バスは一律 -3 dB。全体の出力に音割れ防止（DynamicsCompressor -2 dBFS）。④ 動作音は効果音の同時発音数の予算の対象外（落ちない）。
    - 実測（全体の出力、足音の直後 150 ms の最大 vs 環境音の中央値）: C02 絨毯 +1.5 → +11.7 dB、L12 コンクリート +3.6 → +14.8 dB（15 歩の幅 3.5 dB）、R07 +20.9 → +29.1 dB。
    - 3 回目（Adobe Audition の効果音を採用）: `assets/adobe/audio/foley_footsteps.zip`（download.adobe.com の Audition 用効果音、131.6 MB。Adobe のライセンス: 作品への組み込み可・帰属不要・単体配布不可。git 管理外）があれば、build-footsteps.mjs が連続歩行の録音から一歩ずつ切り出して使う（立ち上がり検出 → 音量が中央値 ±2.5 dB の歩）。硬い床 = 革靴でセメント（コンクリート / タイル / リノリウム / アスファルトは音色だけ加工）、金属 = 革靴で金属の足場、木 = 革靴で木の台、雪 = ブーツで岩塩、着地 = 同じ革靴で各床に飛び降りた音（land.concrete / metal / wood）、草木の擦れ = 茂みを抜ける、麦・背の高い草 = トウモロコシ畑を抜ける（move.rustleTall）。絨毯・草・水・濡れ床は CC0 のまま。無ければ CC0 だけで作る。
    - 切り出しの頭を「最大の打音の立ち上がり」に揃える（録音の部屋の雑音で silenceremove が効かず、先頭に最大 177 ms の間・前触れの擦れの 200 ms 後に本体が来る歩があり、鳴り出しが 0〜230 ms ずれていた → 全床 2〜24 ms）。音量は 100 ms 窓の RMS を -17 dBFS ±0.5 dB に（床ごとの幅 1.5 dB 以内）。自動の持ち上げの上限は +18 dB。
    - 実測（AudioWorklet で全体の出力を取りこぼしなく記録、一歩ごとの 100 ms 窓の最大 vs 環境音の平均。ScriptProcessor は重い部屋で塊を落とすので使わない）: L12 +18.3 dB・一歩ごとの幅 3.6 dB（外れ 1 歩）、C02 +9.4 dB・1.9 dB、R07 +12.2 dB・3.1 dB。
  - **大音量の異常音の修正（第19回。iPhone の画面収録で特定）**: tier low（Convolver なし）のフィードバックディレイ残響（`src/audio/Reverb.ts` DelayReverb）の comb のループ内ローパスが BiquadFilter の既定 Q（1 dB）で遮断周波数付近に +2 dB の山を持ち、フィードバック（最大 0.97）と掛けてループ利得が 1 を超えていた（RT60 1.5 s で 1.09、4 s で 1.19）。録音では約 2.7 kHz の成分が毎秒約 20 dB で増えて振り切れ、30 Hz 間隔の comb の歯のうなりが「高速で反復する」ように聞こえ、数値が Infinity / NaN になると出力全体が無音になっていた。→ ループ内の減衰を利得 ≤ 1 の 1 次ローパス（IIRFilterNode。無ければ Butterworth の biquad）2 本の凸結合にして、ループ利得を常に ≤ 0.97 に。保険として AudioEngine が残響の出力を 0.25 s ごとに見張り、NaN / |x| > 4 なら残響を作り直す（`watchReverb`）。OfflineAudioContext で 10 s 描画: 修正前は RT60 1.5 s で最大 1.5×10⁸ に発散、修正後は減衰。
...
- **設定**（`src/core/Settings.ts` / `src/ui/SettingsPanel.ts`）: 音量 3 本（全体音量の既定 25%。保存済みの設定がある人はその値のまま。画面ノイズの音量連動 `AudioEngine.ambientLevel` は全体音量 25% 以上で旧既定 80% 相当になるよう換算）・視点感度・品質 Tier・描画効果（撮像プリセット。既定 tape）・VHS 効果（0〜200%。既定 200%）・手持ち感（0〜1。既定 0.6、0 で無効）・視線の遅れ（既定 on）・表示 fps（プリセットに従う / 30 / 24 に間引く。tape 以外でも指定可）・REC 表示（既定 on）・トーンマップ（開発用）を `localStorage 'liminal.settings.v3'`（v2 / v1 から移行。VHS 効果は新しい既定に） に保存し起動時に復元。メニューの「設定」パネル（`#settings-slot`）。カメラ挙動の数値は `docs/film-camera.md`。

### メニュー（第21回で刷新。`src/ui/MenuUI.ts`）
- ビデオカメラの一時停止画面（❚❚ PAUSE・四隅の枠・等幅の OSD 文字・琥珀の差し色・うっすら走査線）。タブ「01 マップ」「02 フロアリスト」「03 設定」。1 / 2 / 3・Q / E でタブ、Esc で再開（ポップアップが開いていればそれを閉じる）。最後に開いたタブを覚える。スマホは全画面・縦スクロール。
- **マップ**（`src/ui/MapPanel.ts`）: 左に「階の目盛り」（上ほど高い階。踏破した階ごとの目盛り、ラベルは 12 階以内なら全部・60 階以内は 5 階ごと・それ以上は 10 階ごと。現在地 ● と選択中は必ず。踏破していない階をまたぐ所は ⋮）。↑↓ / PageUp / PageDown / Home で選び、M で平面 / 3D。平面図に階名は描かない（見出しと目盛りに出す）。キャンバスは表示サイズ × 画素密度で描く。
  - 数十階で崩れた原因と対策（`Map3D.ts`）: フロア間隔が一定（最大 14 セル）で 40 階なら高さ 560 セル → カメラが遠平面 500 の外に出て何も映らず、フロアごとのラベルが重なっていた。→ 選択中のフロアの前後は広く・遠いほど詰める間隔（全体の高さを平面の約 2 倍か 48 セルまで）、遠平面はカメラ距離に合わせる、16 フロアを超えたら選択中 ±2 階と現在フロアだけ色分けで描き他は 1 枚にまとめた簡略表示、階の表示は平面図の横に立てた目盛り軸（ラベルは間引き、画面上で重なるものは毎フレーム隠す）。
- **フロアリスト**: レア度別（Common〜Mythic）に全 112 フロアの格子（番号順）。踏破済みは初めて入ったときの画面（入室 2.5 s 後に 480×270 の JPEG を自動で撮る）、未踏破は「？」。踏破済みを長押し（0.45 s。PC は右クリック・Enter も）で詳細（分類・照明・環境音・特徴・形・つながり・危険度 ■□、初踏破の日時・入室回数・見つけた階）。記録は世界をまたいで残る（`src/ui/FloorCodex.ts`: localStorage `liminal.codex.v1`、画像は IndexedDB `liminal-codex`）。記録を始める前のセーブで踏破済みの部屋はメニューを開いたときに取り込む（画像は次の入室で撮る）。新しいフロアに入ると「新しいフロアを記録」の表示。
- **設定**: 音 / 操作 / 映像 / 開発の見出しで SettingsPanel の項目、映像の品質、記録（この世界で発見した部屋・フロアリストの進み・世界のシード、セーブ / ロード / 新しい世界）、開発（デバッグ表示）。

### 地図（D6）
- `src/map/GridProjector.ts` が実座標から 4 m セルへ投影（Legendary 巨大部屋は上限 80 セル）。**部屋は footprint の実形で描く**（折れ廊下・翼付き部屋の矩形ごと）。多層の巨大部屋は `MapCell.levelSpan` で全フロアに描く（非基準階は薄く、Map3D は厚み分）。ゾーン内訳線。グリフ: crawl（下半分の矩形）/ street・gate（太い扉印）/ water（波線）/ 乗車（▣）/ Seam・far（？）/ 施錠（赤）。ミニマップは現在いるフロアだけ、メニューの「マップ」タブでは左の階の目盛りで選ぶ平面図 + 「全体 3D」（`src/ui/Map3D.ts`）。MapRotation / MapErase は表示側のみ実装済み（`game.mapView.rotation` / `hiddenRoomIds`、または `mapCell.hidden = true`。書き手は Phase 2 の Modifier）。
...
### 第22回の修正（足音・描画・部屋の作り）
- **足音の途切れ・質感の違う一歩**: `tools/build-footsteps.mjs` の切り出しの不具合 3 つ。① 長さを詰めてから頭を大きい打音へずらしていたので、多くの歩が 0.06〜0.2 s に削れ、フェード無しで切れていた（途切れ途切れ）。② 一歩は「踵 → つま先」の 2 打で、2 打が残った歩だけ音色が違った（質感の違う音）。③ 5 ms 包絡の時刻を 0.005 s と丸めていて、20 s の録音の尻で 45 ms ずれ、区間の頭が打音の後ろに来ていた。→ 一番大きい打音に頭を揃え、後ろの打音の手前で切る・「踵が大きい歩 / つま先が大きい歩」の多い方だけ使う・尻は実際の長さで 40 ms フェード・繰り返しの録音の重複を除く。硬い床は 1 打 0.28〜0.4 s に揃った（雪・砂利・擦れは連続音のまま）。床の細い帯（短辺 0.25 m 未満の線・目地）は踏んでも床の音のまま（`Game.surfaceUnderPlayer`）。
- **足音の音量**: プレイヤーの動作音（足音・着地・ジャンプ・擦れ）を一律 -3 dB（`AudioEngine` の `PLAYER_SFX_TRIM`）。2 回目: タイルの「コツコツ」をさらに -3 dB（`STEP_GAIN.tile` 0.7）・高域の持ち上げ +5 → +1.5 dB。水の中の一歩は高い濡れた足音（3 kHz 以上が 62%）を元にしていたので作り直した（3 回目: 流れる水の録音を重ねた版は「洗濯機の音」と指摘され撤回）→ 丸めた踵の接地 + 25 ms 後の短い水しぶき（音程 -2〜-12%・5 kHz 以下・0.35 s で消える）の 6 変種。歩いて水に入る専用の CC0 録音は見つからず（OpenGameArt の該当パックは CC-BY）。水への着地・合成の水の一歩も低く。4 回目（「硬い床をカーペット程度に一律で」）: 硬い床は立ち上がりが鋭く残響への送りが絨毯の 3〜5 倍で、実際の出力（環境音を止め AudioWorklet で記録、一歩の後 400 ms の平均 = 残響込み）で絨毯より +4 dB・最大値 +6 dB だった → 送りを下げ（tile 0.8 → 0.5 など）、`STEP_GAIN` を実測で絨毯に揃えた（全 11 床種が絨毯 ±1 dB。硬い床の最大値は絨毯より 3〜6 dB 低い）。
- **C07 駅の連絡通路の床の明滅**: 床の汚れのデカール（床から 4 mm）の深度のずらし（polygonOffset factor -2）が視線が寝るほど大きく、7 mm の点字帯より手前に描かれて三角形ごとに入れ替わっていた → factor 0（units -2 のまま。`DecalAtlas`）。点字帯・線の箔がある全部屋に効く。
- **L03 倉庫内麦畑**: 麦を床全体の格子から「入口 → 出口の斜めの帯」からの距離で間引いていたため、三角・菱形の畑になっていた → 農道の格子の間の区画（四角）の中だけに、区画の辺に揃えた格子で植える（入口から遠い区画ほど粗く。`InstanceOvergrowth.layoutWheat`）。
- **L14 温室都市の外壁の引き伸ばし**: 外壁の 150 m の窓の帯を 1 枚の窓として「窓の奥の部屋」を描いていた → シェーダで幅 2.4 m 前後の窓に割り、窓ごとに部屋・写真・点灯を変え、境目に方立て（`WindowRoom.ts`。他の長い帯の窓にも効く）。
- **C20 非常階段 / E02 年代階段の暗闇に抜ける扉の穴**: 不具合。行き先の無い扉を世界が刈り取っても（removedSockets）、`VerticalGenerator` だけ開口と枠を残していた → 壁に戻す（抽選の後で除くので残りの出口は変わらない。`tests/vertical-removed-sockets.mjs`）。
- **E09 垂直水面オフィス**: 水は出口の 1 枚だけ（その出口が過去に壁へ戻っていると 0 枚）で、縦の水面は波が動かなかった → 外壁の全面に流れ落ちる水の膜（`waterFilm`。壁の手前 1.2 cm の半透明の板）を張り、縦の水面（水壁・水膜）に「流れ落ちる水」の法線（下へ落ちる細かい波 + 蛇行する縦の筋。`MaterialLibrary` の `liminalFallNormal`）と下向きの流れ。
- **L19 凍結リゾートの氷**: 氷の上の発光する青い半透明の箔を外し、材質 `ice` を手続きの氷面（黒氷と白い濁りのまだら・霜・枝分かれするひびと衝撃点の放射状の割れ・気泡・スケートの擦り傷。法線と粗さも同じ絵から。`src/render/IceTexture.ts`）+ clearcoat の艶に。大理石の水色染め（CC0）は使わない。
- **コインランドリーの洗濯機**: 0.6 × 0.6 × 0.85 m → 業務用の実寸 0.72 × 0.75 × 1.05 m（乾燥機 0.85 m を重ねて上端 1.9 m）。列の間隔・島の奥行き・通路の判定も追従（`furniture.washer`）。
- **M14 照明一本の虚空**: 一本道 → 分かれ道・行き止まり・輪のある歩廊の迷路（5 × 5〜7 × 6 の交点を乱択 Prim の全域木でつなぎ、葉を刈り、輪を足す。終点は入口から最も遠い行き止まり。`CorridorGenerator.generateBridgeMaze`。`data/rooms.json` の M14 に layoutHints `maze`、形「入り組んだ歩廊」。資料の xlsx から JSON を作り直すときは同じ変更が要る）。
- **M04 偽帰還口の外**: 5 cm の厚みに重ねた空・草・白い箱の家の絵 → 扉の開口の板（`outsideView`）に全天球の写真を「地面の平らなドーム」に投影して描く（GroundedSkybox と同じ投影を画素ごとに解析的に。近い地面ほど視点でずれ、遠景は動かない。`src/render/OutsideView.ts`）。写真は `public/textures/outside/outside.jpg`（正距円筒 2 : 1・昼の屋外・撮影高さ 1.6 m 前後）。無いときは手続きで描いた住宅街の全天球（空・積雲・家並み・電柱と電線・道路）。現在の写真は Poly Haven「Bethnal Green Entrance」（CC0・帰属不要。tonemapped JPG 8192 × 4096 を 4096 × 2048 に縮小、2.6 MB。ユーザー承認のうえ取得）。
...
- 実機スマホでの性能計測・しゃがむボタン位置の確認は未実施。品質 Tier の閾値は仮。
